...
def prepare_results(df, exchange_codes = None, currencies = None, quantiles = 20, 
    ...
    if svm:
        df["conviction"] = -df["pred_1"] #Pred_0 is 0 against 1, pred_1 is 0 against 2, pred_2 is 1 against 2
    else:
...
model_name_and_region_list = [f"{model}_{region}" for model in models for region in regions]
...
result_cols = ['date', 'gvkey', 'exchange_code',
       ...
       'pred_class', 'train_file', 'split_year', 'gsector',
       'ggroup', 'gind', 'gsubind', 'market_cap_usd', 'trr_5', 'volume_usd_5', 'volatility_5']

for model_name_and_region in model_name_and_region_list:
    for y_i, year in enumerate(years):
        first_results = True
        if year < 2005:
            if model_name_and_region.split("_")[1] == "Europe" or model_name_and_region.split("_")[1] == "Japan":
                continue
        sys.stdout.write("year:", year, "\n")
        for directory in result_dirs:
            if model_name_and_region in directory and "regime_feature" not in directory and f"test_split_{year}" in directory: 
                sys.stdout.write("directory:", directory, "\n")
                current_results = pd.read_parquet(f"results/{directory}/results.parquet", engine="pyarrow")
                current_results["train_file"] = directory.split("file_")[1][:-5]
                train_file = directory.split("file_")[1][:-5]
                current_results["split_year"] = year
                if "markov_rec" in str(current_results["train_file"][0]):
                    current_results["train_file"] = "markov_rec"
                    train_file = "markov_rec"
                if "markov_exp" in str(current_results["train_file"][0]):
                    current_results["train_file"] = "markov_exp"
                    train_file = "markov_exp"
                ...
                if model_name_and_region.split("_")[1] == "US":
                ...
                elif model_name_and_region.split("_")[1] == "Europe":
                ...
                elif model_name_and_region.split("_")[1] == "Japan":
                ...
                for n_i, min_max_quantiles in enumerate(qt_mc_ranges):
                    ...
                    if model_name_and_region.split("_")[0] == "svm":
                        current_results = prepare_results(current_results, quantiles=n_quantiles, min_date = min_date, 
                                                          ...
                                                          max_market_cap_percentile=max_quantile, svm=True)
                    else:
                        current_results = prepare_results(current_results, quantiles=n_quantiles, min_date = min_date, 
                                                        use_percentile_cap=True, min_market_cap_percentile = min_quantile, 
        ...
        if (y_i == 0 and model_name_and_region.split("_")[1] == "US"):
            all_results_digest = all_train_file_results_digest
            continue
        if ((year == 2005) and (model_name_and_region.split("_")[1] == "Europe")) or ((year == 2005) and (model_name_and_region.split("_")[1] == "Japan")):
            all_results_digest = all_train_file_results_digest
            continue
