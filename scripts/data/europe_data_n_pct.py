...
def main(start_year, finish_year, drop_years=1, annual_fundamentals=True):
    ...
    # KEEP ONLY EUROPEAN CURRENCIES
    europe_currencies = ["EUR", "GBP", "SEK", "PLN", "NOK", 
                     "CHF", "DKK", "BGN", "HUF", "ISK", 
                     "CZK", "RSD", "FIM", "FRF", "ESP", 
                     "NLG", "ITL", "CYP", "LTL", "LVL",
                     "SIT", "ROL", "SKK", "DEM", "GRD",
    ...
    data = data[data["currency"].isin(europe_currencies)]
    ...
    for index, year in enumerate(years):
        ...
        if annual_fundamentals:
            joined_data[joined_data["date"].dt.year == year].to_parquet(f'data/processed/europe/europe_data_{year}_annual_fund_processed_n_pct.parquet', index=False)
        else:
            joined_data[joined_data["date"].dt.year == year].to_parquet(f'data/processed/europe/europe_data_{year}_processed_n_pct.parquet', index=False)
